 ...
 * Every property name this DLL uses ("Text", "LocalText", "SelectedRow",
 * "ScrollLocation", "ScrollExtent", "Size", "Name") is already registered by
 * the client's static initializers, so map-based fallbacks (UIBaseObject's
 * generic mProperties lookup) work too. Do NOT use names the client never
 * constructs itself, and avoid dotted pseudo-properties ("SelectedItem.Text")
 * from this side.
 */
class UILowerString {
public:
...
private:
	...
	// (UILowerStringNamespace::calculateHashWithLower / crctable)
	static uint32_t calculateHashWithLower(const char* string) {
		if (!string || !*string)
			return 0;

		uint32_t crc;

		for (crc = 0xFFFFFFFF; *string; ++string)
