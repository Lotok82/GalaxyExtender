...
| R7 | **Marker ack matching — decided 2026-08-05: exact first, then mask-tolerant.** Compare the normalised marked line against each outstanding claim's composed text; on a miss, accept a mask-tolerant match: same author, same length, characters equal at every position where the received character is not a mask character (`*`). This handles receiving-side profanity filters — the ack can arrive as `[Discord] Bob: ****` even though the relay composed clean text, and that's *deterministic* whenever every online client has the filter on (the default) — without loose matching's spoofed-ack hole (a fabricated `[Discord]` line acking an undelivered claim would turn a duplicate into a silently *lost* message; wrong direction). Assumption to verify in-game during the extension harness work: the client's profanity filter masks per character (length-preserving). If it doesn't, fall back to marker + author + claimed-window matching for the filtered case and accept the small spoof surface — unmatched marked lines are dropped, never forwarded, so a wrong match costs at most one prematurely-acked message. | Exact compare reuses the Stage 1 normalise-for-hash pipeline; the mask-tolerant pass runs on the same normalised strings. |
